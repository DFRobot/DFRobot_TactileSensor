...
```python
  ...
  def set_thld(self, thld):
    '''!
    @brief 设置传感器的阈值
    @param thld 阈值,范围为0-4095，默认值为50
    '''
  ...
  def set_dev_addr(self,addr):
    '''!
    @brief 设置传感器的设备地址
    @param addr 设备地址,地址范围为1-255，0是广播地址，不可写入
    '''

  def set_baudrate(self, baud, stop ,parity):
    '''!
    @brief 设置传感器的串口的参数
    @param baud 波特率、停止位、检验位
    '''

  def set_uart_param(self, stop ,parity):
    '''
    @braief 设置传感器串口参数
    @param stop 停止位,parity 检验位
    '''
